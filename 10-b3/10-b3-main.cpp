...
int memory_speed_test(int op)
{
	...
	cout << "          2、在VS2017下测试时，先打开任务管理器，观察运行过程中本程序占用的内存值" << endl;
	cout << "          3、VS2017下如果内存耗尽，会出现错误提示，此时可以观察任务管理器中本程序占用的内存值" << endl;
	...
	*crc_str = 0; //置为空串
	...
	if (check1 || check2)
		...
		if (op == 0) {
			...
			cout << "    老师的机器运行VS2017编译的程序，大约耗时280-300" << endl;
			cout << "    Linux服务器下运行，大约耗时115-125秒" << endl;
		}
		else {
			cout << "内存性能测试(s1+=str方式)" << endl;
			cout << "    老师的机器运行VS2017编译的程序，大约耗时120-140秒" << endl;
			cout << "    Linux服务器下运行，大约耗时50-60秒" << endl;
		}
		cout << "    【说明】：只有相同编译器下的运行时间才有可比性" << endl;
		cout << "              老师的机器为Intel(R) Core(TM) i5-8250U CPU @1.60GHz  1.80GHz" << endl;
