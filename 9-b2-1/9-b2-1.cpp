...
public:
	...
	double area()
	{
		double s1 = sqrt((p1.getx() - p2.getx()) * (p1.getx() - p2.getx()) + (p1.gety() - p2.gety()) * (p1.gety() - p2.gety()));	//p1和p2之间的距离
		double s2 = sqrt((p2.getx() - p3.getx()) * (p2.getx() - p3.getx()) + (p2.gety() - p3.gety()) * (p2.gety() - p3.gety()));	//p2和p3之间的距离
		double s3 = sqrt((p3.getx() - p1.getx()) * (p3.getx() - p1.getx()) + (p3.gety() - p1.gety()) * (p3.gety() - p1.gety()));	//p3和p1之间的距离
		if (s1 + s2 <= s3 || s2 + s3 <= s1 || s3 + s1 <= s2)	//数据不合法
			return -1;
		else
			return 0.5 * fabs(p1.getx() * p2.gety() + p2.getx() * p3.gety() + p3.getx() * p1.gety() - p1.getx() * p3.gety() - p2.getx() * p1.gety() - p3.getx() * p2.gety());	//三阶行列式求三角形面积
	}
	/* 构造函数 */
